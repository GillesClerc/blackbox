...
- Flux reseau box↔cloud : `cloud_client` (challenge→auth→sync HTTPS) **valide sur cible** (jalon F1 du plan `docs/plans/firmware-cloud-client.md`). API URL via `CONFIG_ESCAPEBOX_API_URL`, surcharge NVS `cloud/api_url`. Reste : download scenarios SD (F2), scores (F3), BLE + option B (F4), OTA (F5).
