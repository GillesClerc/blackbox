...
- Flux reseau box↔cloud : `cloud_client` (challenge→auth→sync HTTPS) **valide sur cible** : packages d'assets versionnes installes sur SD (manifest sha256/fichier, install incrementale avec reprise, download authentifie) — jalons F1-F3 du plan `docs/plans/firmware-cloud-client.md`. API URL via `CONFIG_ESCAPEBOX_API_URL`, surcharge NVS `cloud/api_url`. Reste : mixer audio (F4), BLE + option B (F5), OTA (F6) ; scores optionnels.
