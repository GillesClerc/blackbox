...
esp_err_t mtch2120_init(void) {
    ...
    esp_err_t add_ret = i2c_master_bus_add_device(hal_i2c_bus_handle(), &dev_cfg, &s_dev);
    if (add_ret != ESP_OK) return add_ret;

    // Vérifier que le chip répond (lecture du status initial)
    uint8_t buf[2];
    esp_err_t ret = hal_i2c_bus_read_regs(s_dev, REG_DET_STATUS_0, buf, 2);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "MTCH2120 non détecté");
