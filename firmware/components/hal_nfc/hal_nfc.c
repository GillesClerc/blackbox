...
esp_err_t hal_nfc_init(void)
{
    ...
    esp_err_t ret = i2c_master_bus_add_device(hal_i2c_bus_handle(), &dev_cfg, &s_dev);
    if (ret != ESP_OK) {
...
esp_err_t hal_nfc_read_tag(hal_nfc_tag_t *out, uint32_t timeout_ms)
{
...
bool hal_nfc_tag_present(void)
{
    hal_nfc_tag_t tag;
    return hal_nfc_read_tag(&tag, 150) == ESP_OK;
}
