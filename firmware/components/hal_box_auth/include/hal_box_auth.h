...
// dans la partition NVS dédiée "box_nvs" (namespace "box_creds") par
// tools/provision_box.py, qui le dérive du BOX_MASTER_SECRET serveur via HKDF.
// Le firmware ne fait que le lire et signer. box_nvs est séparée de la NVS
// applicative : l'effacement de celle-ci (config_manager) ne touche pas
// l'identité. Les box provisionnées avant box_nvs sont migrées au boot.
//
...
// Charge box_uid + secret depuis box_nvs (migre l'ancien emplacement si
// besoin). À appeler après l'init NVS (config_manager_init). Retourne ESP_ERR_NVS_NOT_FOUND si la box n'a pas été
