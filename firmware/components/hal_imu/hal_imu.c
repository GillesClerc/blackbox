...
esp_err_t hal_imu_init(uint8_t addr) {
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(hal_i2c_bus_handle(), &dev_cfg, &s_dev));

    // WHO_AM_I
    uint8_t who;
    ESP_ERROR_CHECK(hal_i2c_bus_read_reg(s_dev, REG_WHO_AM_I, &who));
    if (who != 0x6C) {
    ...
    // Reset software
    ESP_ERROR_CHECK(hal_i2c_bus_write_reg(s_dev, REG_CTRL3_C, 0x01));
    vTaskDelay(pdMS_TO_TICKS(10));

    // Config
    ESP_ERROR_CHECK(hal_i2c_bus_write_reg(s_dev, REG_CTRL3_C, CTRL3_C_INIT));
    ESP_ERROR_CHECK(hal_i2c_bus_write_reg(s_dev, REG_CTRL1_XL, CTRL1_XL_104HZ_2G));
    ESP_ERROR_CHECK(hal_i2c_bus_write_reg(s_dev, REG_CTRL2_G,  CTRL2_G_104HZ_250));
...
esp_err_t hal_imu_read(hal_imu_data_t *out) {
    ...
    esp_err_t ret = hal_i2c_bus_read_regs(s_dev, REG_TEMP_L, raw, 14);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "hal_imu_read: I2C %s", esp_err_to_name(ret));
        return ret;
    }
...
esp_err_t hal_imu_sleep(void) {
    ...
    esp_err_t ret = hal_i2c_bus_write_reg(s_dev, REG_CTRL1_XL, 0x00);
    if (ret != ESP_OK) { ESP_LOGW(TAG, "hal_imu_sleep: I2C %s", esp_err_to_name(ret)); return ret; }
    ret = hal_i2c_bus_write_reg(s_dev, REG_CTRL2_G, 0x00);
    if (ret != ESP_OK) { ESP_LOGW(TAG, "hal_imu_sleep: I2C %s", esp_err_to_name(ret)); return ret; }
    return ESP_OK;
}
...
esp_err_t hal_imu_wake(void) {
    if (!s_dev) return ESP_ERR_INVALID_STATE;
    esp_err_t ret = hal_i2c_bus_write_reg(s_dev, REG_CTRL1_XL, CTRL1_XL_104HZ_2G);
    if (ret != ESP_OK) { ESP_LOGW(TAG, "hal_imu_wake: I2C %s", esp_err_to_name(ret)); return ret; }
    ret = hal_i2c_bus_write_reg(s_dev, REG_CTRL2_G, CTRL2_G_104HZ_250);
    if (ret != ESP_OK) { ESP_LOGW(TAG, "hal_imu_wake: I2C %s", esp_err_to_name(ret)); return ret; }
    return ESP_OK;
}
